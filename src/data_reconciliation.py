...
MONGODB_LOCATION_LOOKUP_COLLECTION = os.environ.get(
    "MONGODB_LOCATION_LOOKUP_COLLECTION"
)
...
def handle_non_reconciled_idealista_neighborhoods(spark: SparkSession):
    idealista_df = (
        ...
        .option("collection", MONGODB_IDEALISTA_COLLECTION)
        .load()
    )
    ...
    location_lookup_df = (
        spark.read.format("mongodb")
        .option("database", MONGODB_DB)
        .option("collection", MONGODB_LOCATION_LOOKUP_COLLECTION)
        .load()
    )

    logger.info(f"Joining idealista with location_lookup")
    location_lookup_df = location_lookup_df.withColumnsRenamed(
        {"neighborhood": "neighborhood_", "district": "district_"}
    )
    ...
    logger.info(
        f"There are {idealista_df_looked_up.filter('neighborhood_id is null').count()} listings with non-reconciled neighborhoods"
    )

    problematic_neighborhoods = (
        idealista_df_looked_up.filter("neighborhood_id is null")
        .select("neighborhood")
        .distinct()
    )
    logger.info(
        f"There are {problematic_neighborhoods.count()} unique non-reconciled neighborhoods"
    )

    jaccard_similarity_udf = udf(jaccard_similarity, DoubleType())

    logger.info(
        f"Calculating jaccard similarity between non-reconciled neighborhoods and location_lookup neighborhoods to find matches"
    )
    ...
    filtered_df = cross_joined_df.filter(
        col("jaccard_similarity") >= similarity_threshold
    )
    ...
    logger.info(
        f"Sorting non-reconciled neighborhoods by jaccard similarity and dropping duplicates"
    )
    non_reconciled_locations = non_reconciled_locations.orderBy(
    ...
    non_reconciled_locations = non_reconciled_locations.withColumn(
        "neighborhood_id",
        when(col("neighborhood") == "El Gòtic", "Q17154").otherwise(
            col("neighborhood_id")
        ),
    )
    ...
    logger.info(
        f"Adding 'El Poble Sec - Parc de Montjuïc' neighborhood with id Q980253 and 'Horta' neighborhood with id Q15225338"
    )
    ...
    non_reconciled_locations.write.format("mongodb").option(
        "database", MONGODB_DB
    ).option("collection", "idealista_resolved_location_lookup").mode(
        "overwrite"
    ).save()
