...
from data_formatters import (AirQualityDataSource, AirQualityLookupDataSource,
                             IdealistaDataSource, IncomeDataSource,
...
MONGODB_LOCATION_LOOKUP_IDENTIFIERS = os.environ.get(
    "MONGODB_LOCATION_LOOKUP_IDENTIFIERS"
).split(",")
MONGODB_AIRQUALITY_LOOKUP_COLLECTION = os.environ.get(
    "MONGODB_AIRQUALITY_LOOKUP_COLLECTION"
)
MONGODB_AIRQUALITY_LOOKUP_IDENTIFIERS = os.environ.get(
    "MONGODB_AIRQUALITY_LOOKUP_IDENTIFIERS"
).split(",")
...
if __name__ == "__main__":
    ...
    logger.info(
        f"Starting the pipeline at {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(pipeline_start_time))}"
    )
    for source in sources:
        ...
        logger.info(
            f"Finished processing {source_name} in {int(minutes)} minutes and {int(seconds)} seconds"
        )
    ...
    logger.info(
        f"Finished the reconciliation process in {int(minutes)} minutes and {int(seconds)} seconds"
    )

    minutes, seconds = divmod(time.time() - pipeline_start_time, 60)
    logger.info(
        f"Finished the formatted pipeline in {int(minutes)} minutes and {int(seconds)} seconds"
    )
    spark.stop()
